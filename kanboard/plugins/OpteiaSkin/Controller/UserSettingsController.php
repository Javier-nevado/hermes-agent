...
class UserSettingsController extends UserViewController
{
    public function show(){
        $user = $this->getUser();
        $this->response->html($this->helper->layout->user('OpteiaSkin:user/theme', array(
            'user' => $user,
    ...
    public function save(){
        ...
        if ($this->userSession->getId() == $user['id']){
            ...
            $this->response->redirect($this->helper->url->to('UserSettingsController', 'show', array('plugin' => 'OpteiaSkin', 'user_id' => $user['id'])));
        }
    }
}
