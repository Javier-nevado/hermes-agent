...
class CustomColorModel extends ColorModel
{
    ...
    {
        if (isset($GLOBALS['opteiaSkinConfig']) && !$GLOBALS['opteiaSkinConfig']['overwrite_default_task_color']){
    ...
    {
        $buffer = '';
        if (isset($GLOBALS['opteiaSkinConfig'])){
            $buffer .= ':root{';
    ...
    private function getCssString($color){
        ...
        foreach ($GLOBALS['opteiaSkinConfig'][$color.'_palette'] as $cssName => $cssValue) {
