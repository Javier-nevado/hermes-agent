...
class BasePlatformAdapter(ABC):
    ...
    def extract_media(content: str) -> Tuple[List[Tuple[str, bool]], str]:
        ...
        media_pattern = re.compile(
            r'''[`"']?MEDIA:\s*(?P<path>`[^`\n]+`|"[^"\n]+"|'[^'\n]+'|(?:~/|/)\S+(?:[^\S\n]+\S+)*?\.(?:png|jpe?g|gif|webp|mp4|mov|avi|mkv|webm|ogg|opus|mp3|wav|m4a|flac|epub|pdf|zip|rar|7z|docx?|xlsx?|pptx?|txt|csv|apk|ipa|html?|md|json|ya?ml|py|sh|ps1|bat)(?=[\s`"',;:)\]}]|$))[`"']?'''
        )
        ...
        try:
            ...
            if response:
                ...
                try:
                    ...
                    logger.warning("[%s] ABI auto-deliver check failed: %s", self.name, _abi_err)
